...
const RepositoryCard: React.FC<{ repo: IRepository }> = ({ repo }) => {
  const linkUrl = repo.homepage && repo.homepage.length ? repo.homepage : repo.html_url
  let homepageHost: string | null = null
  try {
    homepageHost = repo.homepage ? new URL(repo.homepage).host : null
  } catch (e) {
    homepageHost = null
  }
    ...
    <div className="relative flex flex-col overflow-hidden bg-white/50 backdrop-blur dark:bg-black/80">
      <div className="flex-1 p-5">
        ...
        <div className="block h-10 text-xs font-bold">
          ...
          <br />
          {homepageHost && (
            <a href={linkUrl} target="_blank" rel="noopener noreferrer" className="hover:text-yellow-600">
              {homepageHost}
            </a>
          )}
        </div>
