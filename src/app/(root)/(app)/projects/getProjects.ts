...
// Request up to 100 repos to avoid pagination hiding repos on the 2nd+ page
const repositoriesUrl = `https://api.github.com/users/${username}/repos?per_page=100&sort=updated&visibility=public&affiliation=owner`
...
const getProjects = async (): Promise<IRepository[]> => {
  ...
  // Exclude the "self" repo (the repo named the same as the username)
  // and keep only repos that have a description and a languages_url
  repositories = repositories.filter((repo) => repo.name !== username && repo.description && repo.languages_url)
